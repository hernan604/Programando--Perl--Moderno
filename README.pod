...
O lib recebe uma lista de diretórios onde tem módulos que precisam ser carregados. Portanto é possível fazer algo assim:
    ...
    use lib '../Modulo1/lib/','../Modulo2/lib/';
    ..codigo..
...
Os argumentos sempre chegam na lista @_. O @_ começa com @ portanto é uma lista. É uma variável especial que representa a lista de argumentos recebida. Então é possível criar uma lista de variáveis  equalizar com a lista recebida, isto fará cada variável conter o valor que chegou, ex: 

    
    my @argumentos = ( 'argumento_1', 'argumento_2' );
    my ( $__arg1, $__arg2 ) = @argumentos;   # equaliza lista da esquerda com a da direita
    warn $__arg1;   #printa o valor referente à lista da direita no momento da atribuição
    warn $__arg2;   #printa o valor referente à lista da direita no momento da atribuição

    my $metodo     = sub { 
        my ( $arg1, $arg2 ) = @_; #equaliza a lista de argumentos de entrada (@_) com a lista da esquerda
        warn "ARG1: ". $arg1;   # printa o valor referente
        warn "ARG2: ". $arg2;   # printa o valor referente
    };
    $metodo->( @argumentos ); #passa a lista como argumento
...
Entre as variáveis especiais no perl, existe a @INC e %INC. Elas são o caminho das pedras para o interpretador encontrar os módulos na hora de executar uma aplicação perl.
